...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///D:\FPT_University\Spring_2023\AIP391\Test\Database/List_Motel.db'
# app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///D:\FPT_University\Spring_2023\AIP391\Test\Database/Waiting_cf.db'
db = SQLAlchemy(app)
...
def predict():
    ...
    # filename = get_files('D:\FPT_University\Spring_2023\AIP391\Demo\Account_Management\Account.txt')
    account_list = list_name("Account","Account","Account")
    filename = account_list[len(account_list)-1]
    filename_s = f"{filename}.txt"
    save_file_comunication('D:\FPT_University\Spring_2023\AIP391\Test\Comunication', filename_s,
                           '%s: %s' %(filename, text))
...
def login_admin():
    if request.method == 'POST':
        ...
        if tro:
            ...
            error_message = "Please! Re-enter ID Roomname!"
            return render_template('index_admin.html', error_message=error_message)
...
def add_accoount():
    if request.method == 'POST':
        ...
        if account in list_wait:
            ...
            error_message = "New ID invalid! Re-enter!"
            return render_template('add_account.html', error_message=error_message)
